...
from utils import tsne_interface, lexicon_helper
...
def get_text_sequences(text_file_path, vocab_size, vocab_save_path):
    word_index = deepcopy(global_config.predefined_word_index)
...
def get_test_sequences(text_file_path, text_tokenizer, word_index, inverse_word_index):
    ...
    actual_word_lists = [generate_words_from_indices(x, inverse_word_index) for x in actual_sequences]
...
def get_labels(label_file_path, store_labels, one_hot_encode=True):
    ...
    if one_hot_encode:
        ...
        return [np.asarray(all_labels), num_labels]
