...
parser.add_argument('--s', type=int, default='50000', help='number of s to generate')
...
ns = len(corpus.dictionary)
hidden = model.init_hidden(1)
input = torch.randint(ns, (1, 1), dtype=torch.long).to(device)

with open(args.outf, 'w') as outf:
    with torch.no_grad():  # no tracking history
        for i in range(args.s):
            output, hidden = model(input, hidden)
            s_weights = output.squeeze().div(args.temperature).exp().cpu()
            s_idx = torch.multinomial(s_weights, 1)[0]
            input.fill_(s_idx)
            s = corpus.dictionary.idx2word[s_idx]
            outf.write(s + ('\n' if i % 50000 == 49999 else ' '))
            if i % args.log_interval == 0:
                print('| Generated {}/{} words'.format(i, args.s))
